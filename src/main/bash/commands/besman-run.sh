...
function __bes_run() {
    local playbook_name playbook_version playbook_file playbook_file_local steps_file_local steps_file_base_name
    ...
    if [[ -n "$BESMAN_LOCAL_PLAYBOOK" && "$BESMAN_LOCAL_PLAYBOOK" == "true" ]]; then
        ...
        if [[ -n "$BESMAN_LOCAL_PLAYBOOK_DIR" ]]; then
            ...
            steps_file_base_name="besman-$playbook_name-steps-$playbook_version"
            steps_file_local="$BESMAN_LOCAL_PLAYBOOK_DIR/playbooks/$(find "$BESMAN_LOCAL_PLAYBOOK_DIR/playbooks" -maxdepth 1 -type f -regex ".*/$steps_file_base_name\.\(sh\|md\|ipynb\)" | head -n 1 | xargs basename)"
            ...
            if [[ ! -f "$steps_file_local" ]]; then
                __besman_echo_red "Error: Local playbook file not found: $steps_file_local"
                return 1
            fi
            ...
            if [[ -n "$BESMAN_PLAYBOOK_DIR" ]]; then
                ...
                cp -f "$steps_file_local" "$BESMAN_PLAYBOOK_DIR" || { # -f for force overwrite
                    __besman_echo_red "Error: Failed to copy playbook to $BESMAN_PLAYBOOK_DIR"
                    return 1
                }
